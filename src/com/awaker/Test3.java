...
public class Test3 {
    public static void main(String[] args) {
        try {
            InputStream is = new FileInputStream("media/music.mp3");
    ...
    /*final*/ private Decoder decoder;
        ...
        public boolean play(int frames) throws JavaLayerException {
            ...
            while (frames-- > 0 && ret) {
                ret = decodeFrame();
                System.out.println(((new Date()).getTime() - startTime) + " : " + count);
                count++;
            }
        ...
        protected boolean decodeFrame() throws JavaLayerException {
            try {
                ...
                Complex[] comp = new Complex[1024];

                for (int i = 0; i < 1024; i++) {
                    real[i] = samples[i * 2];
                    imag[i] = 0;
                    comp[i] = new Complex(samples[i * 2], 0);
                }

                s.fft(real, imag);

                Complex[] fftout = com.awaker.princeton.FFT.fft(comp);

                boolean setClipboard = false;
                if (setClipboard) {
                    Clipboard cb = Toolkit.getDefaultToolkit().getSystemClipboard();

                    String ss = "";
                    for (Complex c : fftout) {
                        ss += c.re() + "\n";
                    }
